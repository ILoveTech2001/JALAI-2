...
const OrphanageDashboard = () => {
  ...
  // Dummy requests (replace with backend data)
  const requests = [
    { name: "Bei Asheli", details: "Wants to donate clothes" },
    { name: "Marie Agathe", details: "Offering ₦5,000 cash" },
  ];

  // Example: Fetch stats from backend
  useEffect(() => {
    // Uncomment and update URL when backend is ready
    // axios.get("/api/orphanage/stats").then(res => setStats(res.data));
    // For now, simulate stats:
    setStats({
      totalCashDonations: 12,
      totalKindDonations: 23,
      totalRequests: 5,
      pendingMessages: 3,
    });
  }, []);
