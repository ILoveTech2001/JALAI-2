...
function Dashboard() {
  ...
  const fetchOrphanages = async () => {
    try {
      ...
      console.log('Homepage orphanages response:', response)
      ...
      // Transform backend data for homepage display
      const transformedOrphanages = orphanagesData.map(orphanage => ({
        id: orphanage.id,
        name: orphanage.name,
        location: orphanage.location,
        locationLabel: orphanage.location,
        address: orphanage.location,
        contact: orphanage.contactPerson || 'Director',
        phone: orphanage.contactPhone,
        capacity: orphanage.capacity,
        children_count: orphanage.currentChildren,
        description: orphanage.description,
        image: orphanage.imageUrl || '/assets/orphanage-default.jpg',
        established: new Date(orphanage.createdAt).getFullYear(),
        needs: orphanage.needsDescription?.split(',') || ['General support'],
        urgentNeeds: orphanage.needsDescription?.split(',').slice(0, 2) || ['Support needed']
      }))

      setOrphanagesData(transformedOrphanages)
      setError(null)
    } catch (error) {
  ...
  const filtered = orphanagesData.filter(o =>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden">
            <CardContent className="p-6 text-center space-y-2">
              ...
              <h3 className="text-2xl font-bold text-green-800">{orphanagesData.length}+</h3>
          ...
          {loading ? (
            <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-green-600">Please wait while we fetch the latest information</p>
              </CardContent>
            </Card>
          ) : error ? (
            <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden">
              <CardContent className="p-12 text-center">
                ...
                <Button onClick={fetchOrphanages} className="bg-green-600 hover:bg-green-700">
                  Try Again
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden">
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-green-800 mb-2">No orphanages found</h3>
                <p className="text-green-600">
                  {query ? 'Try adjusting your search terms' : 'No verified orphanages available at the moment'}
                </p>
              </CardContent>
            </Card>
          ) : (
